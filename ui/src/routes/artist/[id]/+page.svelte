<script lang="ts">
	...
	function displaySectionTitle(title: string) {
		...
		if (name && title.trim().toLocaleLowerCase() === name.toLocaleLowerCase()) return `More like ${name}`;
		return title;
	}

	function supportedSection(title: string) {
		return !/music\s*videos?|video\s+for\s+you/i.test(title);
	}
...
{:else if artist}
	...
	<div class="content-in relative flex min-h-[45vh] flex-col justify-end overflow-hidden">
		{#if artist.thumbnail}
			<img src={thumb(artist.thumbnail, 640)} alt="" class="absolute inset-0 h-full w-full object-cover" />
		{/if}
		<div
	...
	<div class="content-in flex flex-col gap-8 p-6">
		...
		{#each artist.sections.filter((section) => supportedSection(section.title)) as section, i (i + ':' + section.title)}
			<Shelf
