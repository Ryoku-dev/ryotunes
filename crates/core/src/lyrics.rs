...
/// `RYOTUNES_LYRICS_ONLY=<boidu|netease|qq|kugou>` pins the chain to that one provider and bypasses
...
fn forced_provider() -> Option<String> {
    std::env::var("RYOTUNES_LYRICS_ONLY").ok().filter(|s| !s.is_empty())
}
...
async fn fetch(state: &AppState, mut req: LyricsRequest) -> (Option<Lyrics>, bool) {
    ...
    if let Some(only) = forced_provider() {
        let hit = match only.as_str() {
            ...
            other => {
                tracing::warn!(provider = other, "RYOTUNES_LYRICS_ONLY: unknown provider");
                Ok(None)
            }
        };
        match &hit {
...
async fn boidu_get(req: &LyricsRequest) -> Result<Option<Lyrics>, reqwest::Error> {
    ...
    tracing::debug!("lyrics: querying Boidu provider");
...
mod tests {
    ...
    /// run (network test policy: network tests are opt-in, or `cargo test` fails offline):
    ///   cargo test -p ryotunes --lib -- --ignored --nocapture
    ///
